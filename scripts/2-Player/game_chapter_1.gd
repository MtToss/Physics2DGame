extends Node2D
@onready var camera2D = $Character_Handler/ENUMAN/Camera2D

@onready var enuman_collision_shape = $Character_Handler/ENUMAN/CollisionShape2D
@onready var dog_collision_shape = $Character_Handler/Doggi/CollisionShape2D
...
@onready var line_edit = $CanvasLayer/problem_panel/Panel/LineEdit
...
func _ready() -> void:
	...
	$Area2D.connect("body_entered", Callable(self,"teleporter_body"))
